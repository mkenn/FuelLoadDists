...
for(j in evt.tallies$evt.min_tally)
{
  ...
  range.vals<-samo.fn(data.file=AllLoads.df,evts=cur.evt,start.col=12,
                      cur.cols=c(12:ncol(AllLoads.df)),evt.col="LFEVTGroupCd",min.n=30,q.lower=0.1,q.upper=0.9,write.file=FALSE,file.name="SensitivityAnalysis_")
  ...
  {  
    ...
    {  
      ...
      {
        sens.list[[k]]<-tell(fuelLoadFiles[[list.id]][[2]],results.sa[[list.id]][,response.vars[k]])
