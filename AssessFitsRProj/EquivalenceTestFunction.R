                         ...
                         ep.val=.25, include0=FALSE,add.val=0.01)
{
  ...
  for(i in 1:length(evts))
  {
    ...
    equivalence.list[[i]]<-data.frame(fueltype=names(data.file)[cur.cols],normResult=NA,lnormResult=NA,
                                      gammaResult=NA,weibullResult=NA) # 0 indicates not rejected, 1 rejected. Looking for distributions with 1's
    for(j in 1:length(cur.cols))
    {
      if(!is.na(distFit.obj[[i]][j,2])) # so we have fit statistics
      {
        ...
        norm.cdf<-pnorm(cur.loads.vals,mean=distFit.obj[[i]]$norm.mu[j],
