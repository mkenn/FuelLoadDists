...
for(i in 1:length(evt.vals))
{
  ...
  for(j in 1:nrow(distributionCustomRankingHurdleNOut[[i]]))
  {
    if(!is.na(distributionCustomRankingHurdleNOut[[i]]$dist1.fit[j])&!is.na(fuel.names$DataBaseInR[j]))
    {
      ...
      distFitForPubGraph.fn(data.file=data.file,evts=evt.vals[i],cur.cols = j+start.col-1,evt.col = EVTCol,
                            distr=cur.distr,
                            plot.qqAxes = FALSE,x.lab=as.character(fuel.names$graphLabelNoUnits[j]),#,x.lab = distributionCustomRankingHurdle[[i]]$fueltype[j],
                            main.txt=paste(as.character(evt.names$EVT_Group_AbbreviatedName[evt.names$EVT_GP==evt.vals[i]]),"; ",
                                           round(distributionCustomFittingHurdleNOut$HurdleFit[[i]]$prop0[j]*100,digits=1),"% zero; ",
                                           cur.distr,sep=""),removeOut=TRUE)
      dev.off()
...
#########
# all fit graphs including outliers in the fits
pdf(file="HurdleFitGraphs/InclOutliers/AllHurldeFitsWithOutliersInFit.pdf",height=3.5,width=10)
for(i in 1:length(evt.vals))
{
  #  tmp.df<-data.file[data.file[,EVTCol]==evt.vals[i],]
  #  for(j in start.col:ncol(data.file))
  for(j in 1:nrow(distributionCustomRankingHurdle[[i]]))
  {
    if(!is.na(distributionCustomRankingHurdle[[i]]$dist1.fit[j])&!is.na(fuel.names$DataBaseInR[j]))
    {
      cur.distr<-switch(distributionCustomRankingHurdle[[i]]$dist1.fit[j],
                        gammaLL="gamma",
                        lnormLL="lnorm",
                        normLL="norm",
                        weibullLL="weibull")
      distFitForSupplementaryGraph.fn(data.file=data.file,evts=evt.vals[i],cur.cols = j+start.col-1,
                                      evt.col = EVTCol,
                                      distr=cur.distr,
                                      plot.qqAxes = TRUE,x.lab=as.character(fuel.names$graphLabelNoUnits[j]),#x.lab = distributionCustomRankingHurdle[[i]]$fueltype[j],
                                      main.txt=paste("EVT",evt.vals[i],cur.distr))
    }
  }
