...
#  for(j in start.col:ncol(data.file))
    ...
    {
     if(!is.na(distributionCustomRankingHurdle[[i]]$dist1.fit[j]))
     {
        postscript(file=paste("HurdleFitGraphs/IncludeOutliers/WithQQAxis/EVT",evt.vals[i],"fuelRow",j,".eps",sep=""),
...
for(i in 1:length(evt.vals))
{
  ...
  for(j in 1:nrow(distributionCustomRankingHurdle[[i]]))
  {
    if(!is.na(distributionCustomRankingHurdle[[i]]$dist1.fit[j]))
    {
      postscript(file=paste("HurdleFitGraphs/IncludeOutliers/EVT",evt.vals[i],"fuelRow",j,"NoQQAxis.eps",sep=""),
                 horizontal=FALSE,height=3.5,width=4,onefile=FALSE)
      cur.distr<-switch(distributionCustomRankingHurdle[[i]]$dist1.fit[j],
                        gammaLL="gamma",
                        lnormLL="lnorm",
...
pdf(file="HurdleFitGraphs/IncludeOutliers/AllGraphs.pdf",height=3.5,width=10)
for(i in 1:length(evt.vals))
{
  #  tmp.df<-data.file[data.file[,EVTCol]==evt.vals[i],]
  #  for(j in start.col:ncol(data.file))
...
pdf(file="HurdleFitGraphs/IncludeOutliers/AllGraphsWithWeibull.pdf",height=3.5,width=10)
for(i in 1:length(evt.vals))
{
  #  tmp.df<-data.file[data.file[,EVTCol]==evt.vals[i],]
  #  for(j in start.col:ncol(data.file))
